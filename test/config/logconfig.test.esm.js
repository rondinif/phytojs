...
test('logconfig library should be loaded.', assert => {
	assert.plan(1);
	assert.ok(logconfig, 'the logconfig library loaded');
});

test('logconfig has a function named isLogSilent which has no arguments and return a boolean indicating when test features are enabled for the current execution environment.', assert => {
	assert.plan(2);
...
test('logconfig has a function named isLogVerbose which has no arguments and return a boolean indicating when test features are enabled for the current execution environment.', assert => {
	assert.plan(2);
