...
test('isLogSilent() should respond even if`IS_LOG_SILENT` environment setting is missing.', assert => {
...
test('isLogVerbose() should respond even if`IS_LOG_VERBOSE` environment setting is missing.', assert => {
	const sandbox = sinon.createSandbox();
	sandbox.stub(process, 'env').value({});
...
test('isLogSilent() should respond consistently with `IS_LOG_SILENT` environment setting.', assert => {
	const sandbox = sinon.createSandbox();
