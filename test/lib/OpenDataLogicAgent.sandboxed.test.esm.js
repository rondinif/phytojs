...
nocktest('getWdEndpointUri should return a live wikidata.org public cloud API service when running in production (i.e: not IS_UNDER_TEST=false)', async assert => {
	// Arrange
...
nocktest('getWdEndpointUri should return the test double `virtual service` (ie: mountebank 127.0.0.1:6568), when not running in production mode (i.e: IS_UNDER_TEST=true)', async assert => {
	// Arrange
	const sandbox = sinon.createSandbox();
	// NB: this is not possible => sandbox.stub(phyto, 'getPromiseOfSparqlGetSpecieArticleByEntityId').returns(Promise.rejects("simulated rejection"));
...
nocktest('getSparqlEndpointUri should return a live cloud `Wikidata Query Service` when running in production (i.e: not IS_UNDER_TEST=false)', async assert => {
	// Arrange
	const sandbox = sinon.createSandbox();
	// NB: this is not possible => sandbox.stub(phyto, 'getPromiseOfSparqlGetSpecieArticleByEntityId').returns(Promise.rejects("simulated rejection"));
	sandbox.stub(config, 'isUnderTest').value(() => false);
...
nocktest('getSparqlEndpointUri should return the test double `virtual service` (ie: mountebank 127.0.0.1:6568), when not running in production mode (i.e: IS_UNDER_TEST=true)', async assert => {
	// Arrange
	const sandbox = sinon.createSandbox();
	// NB: this is not possible => sandbox.stub(phyto, 'getPromiseOfSparqlGetSpecieArticleByEntityId').returns(Promise.rejects("simulated rejection"));
	sandbox.stub(config, 'isUnderTest').value(() => true);
