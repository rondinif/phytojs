...
// UNUSED ? import fetch from 'isomorphic-fetch';
...
/*
 * TEST: wdSearchByAnyName_simulated_error_TODO_give_a_better_name
*/
nocktest('wdSearchByAnyName_simulated_error_TODO_give_a_better_name', async assert => {
	// Before All @see https://gist.github.com/MatthewKosloski/0e3a44f49e8ba39d491af644552230c7
 ...
 * TEST: wdSearchByAnyName_simulated_error_TODO_give_a_better_name
*/
nocktest('sparqlScientificNameById_simulated_error_TODO_give_a_better_name', async assert => {
	// Before All @see https://gist.github.com/MatthewKosloski/0e3a44f49e8ba39d491af644552230c7
	// const sandbox = sinon.createSandbox();
	// sandbox.stub(fetch).value( () => {
...
nocktest('resolvedPlantsByName_simulated_error_TODO_give_a_better_name', async assert => {
	// Before All @see https://gist.github.com/MatthewKosloski/0e3a44f49e8ba39d491af644552230c7
	// const sandbox = sinon.createSandbox();
	// sandbox.stub(fetch).value( () => {
	const fakeFetch = stub();
	const fakeResponse = Promise.resolve({
		status: 400,
		json: () => Promise.reject(new Error('simulated rejection'))
	});
	fakeFetch.withArgs().returns(fakeResponse);
