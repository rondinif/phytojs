...
test('log library should be loaded.', assert => {
	assert.plan(1);
	assert.ok(log, 'the log library loaded');
	assert.end();
});

test('log has functions named: tarce,debug,info,warn,error', assert => {
...
test('log.trace should use console.trace when log is verbose', assert => {
	const sandbox = sinon.createSandbox();
	sandbox.stub(process, 'env').value({IS_LOG_SILENT: 'false', IS_LOG_VERBOSE: 'true'});
...
test('log.trace should not use console.trace when log is silent', assert => {
	const sandbox = sinon.createSandbox();
...
test('log.debug should use console.debug when log is verbose', assert => {
	const sandbox = sinon.createSandbox();
	sandbox.stub(process, 'env').value({IS_LOG_SILENT: 'false', IS_LOG_VERBOSE: 'true'});
...
test('log.debug should not use console.debug when log is silent', assert => {
	const sandbox = sinon.createSandbox();
	sandbox.stub(process, 'env').value({IS_LOG_SILENT: 'true', IS_LOG_VERBOSE: 'false'});
...
test('log.info should use console.info when log is verbose', assert => {
	const sandbox = sinon.createSandbox();
	sandbox.stub(process, 'env').value({IS_LOG_SILENT: 'false', IS_LOG_VERBOSE: 'true'});
...
test('log.info should not use console.info when log is silent', assert => {
	const sandbox = sinon.createSandbox();
	sandbox.stub(process, 'env').value({IS_LOG_SILENT: 'true', IS_LOG_VERBOSE: 'false'});
...
test('log.warn should use console.warn when log is verbose', assert => {
	const sandbox = sinon.createSandbox();
...
test('log.warn should not use console.warn when log is silent', assert => {
	const sandbox = sinon.createSandbox();
	sandbox.stub(process, 'env').value({IS_LOG_SILENT: 'true', IS_LOG_VERBOSE: 'false'});
...
test('log.error should use console.error', assert => {
	const sandbox = sinon.createSandbox();
	sandbox.stub(process, 'env').value({IS_LOG_SILENT: 'false', IS_LOG_VERBOSE: 'true'});
...
test('log.error should use console.error even if logsilent is true', assert => {
	const sandbox = sinon.createSandbox();
	sandbox.stub(process, 'env').value({IS_LOG_SILENT: 'true', IS_LOG_VERBOSE: 'false'});
...
test('isLogSilent() and isLogVerbove cannot be both true.', assert => {
	const sandbox = sinon.createSandbox();
